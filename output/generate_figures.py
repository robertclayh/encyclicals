...
# Precomputed TFIDF summaries (generated by output/precompute_tfidf_summaries.py)
# The full TFIDF matrix (17k docs x 68k terms) is too large for CSV storage;
# these files contain only the per-figure aggregates needed.
TFIDF_FIG11 = pd.read_csv(PROCESSED / "TFIDF_FIG11.csv", index_col="term_str")
TFIDF_FIG12 = pd.read_csv(PROCESSED / "TFIDF_FIG12.csv")

LIBRARY["year_num"] = pd.to_numeric(LIBRARY["year"], errors="coerce")

# ── Language filter ────────────────────────────────────────────────────────────
# The corpus was built with ENGLISH_ONLY=True, so all documents are English
# by construction. All docs are considered language-valid.
LIBRARY["lang_ok"] = True
print(f"Corpus size: {len(LIBRARY):,} docs (English-only corpus)")
...
# ─────────────────────────────────────────────────────────────────────────────
print("Fig 11: vocab shift (English-only corpus, pre/post Vatican II)...")
# Load precomputed per-term mean TF-IDF from output/precompute_tfidf_summaries.py
pre_mean  = TFIDF_FIG11["pre_mean"].nlargest(15)
post_mean = TFIDF_FIG11["post_mean"].nlargest(15)

# Recover doc counts from LIBRARY directly
eng_pre  = LIBRARY[(LIBRARY["year_num"] <= 1961) & LIBRARY["lang_ok"]].index.tolist()
eng_post = LIBRARY[(LIBRARY["year_num"] >  1965) & LIBRARY["lang_ok"]].index.tolist()
...
# Load precomputed term presence from output/precompute_tfidf_summaries.py
available = [t for t in social_terms if t in TFIDF_FIG12["term_str"].values]
...
for ti, term in enumerate(available):
	docs_with = TFIDF_FIG12.loc[TFIDF_FIG12["term_str"] == term, "doc_id"]
